...
def loss_of_one_batch_2v(gt_num_image, batch, model, criterion, device, symmetrize_batch=False, use_amp=False, ret=None):
    ...
    with torch.cuda.amp.autocast(enabled=bool(use_amp), dtype=dtype):
        ...
        # MESSING UP CAMERA POSE ON PURPOSE TO SEE WHAT HAPPENS
        #view1[0]['camera_pose'] = torch.eye(4).unsqueeze(0).to(device)
        #view2[0]['camera_pose'] = torch.eye(4).unsqueeze(0).to(device)
        #render_gt[0]['camera_pose'] = torch.eye(4).unsqueeze(0).to(device)
        #print()
        #print('message from dust3r/inference/loss_of_one_batch_2v')
        #print('messing up the pts3d of view2')
        #view2[0]['pts3d'] = torch.zeros_like(view2[0]['pts3d']) 
        #print(view2[0]['pts3d'])

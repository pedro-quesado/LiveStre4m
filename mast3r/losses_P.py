...
from dust3r.renderers.gaussian_renderer_P import GaussianRenderer_P
...
class Eval_NVS(nn.Module):
    ...
    """
    def __init__(self, alpha, scaling_mode='interp_5e-4_0.1_3', interactive=False):
        super().__init__()
    ...
    def render(self, render_params, image_size):
        #print()
        #print('in Eval_NVS, in def render we get:')
        #print('render_params', type(render_params), len(render_params), render_params)
        ...
        gs_render = GaussianRenderer_P(H_org, W_org, gs_kwargs={'type':self.scaling_mode['type'], 'min_scaling': self.scaling_mode['min_scaling'], 'max_scaling': self.scaling_mode['max_scaling'], 'scaling_factor': scaling_factor}, interactive=self.interactive)
        ...
        #print('images are generated', images.shape)
        #print()
    ...
    def get_all_pts3d(self, name, gt1, gt2, pred1, pred2, trajectory_pred, dist_clip=None, render_gt=None):
        ...
        pr_pts2 = pred2['pts3d']
        #print()
        #print('pr_pts1:',pr_pts1.min(),pr_pts1.mean(),pr_pts1.max())
        #print('pr_pts2:',pr_pts2.min(),pr_pts2.mean(),pr_pts2.max())
        #print()
        # GT pts3d
        ...
        #print('OPACITY shape, max, mean, min', opacity.shape, opacity.max(), opacity.mean(), opacity.min())
        ...
        #print('shape of pr_pts1 is', pr_pts1.shape)
        #print('shape of pr_pts2 is', pr_pts2.shape)
        ...
        #print('shape of xyz is', xyz.shape)
        ...
        with torch.set_grad_enabled(True):
            ...
            for i in range(1): #P if i in range(1) than it works, but the image is just black
                extrinsics = extrinsics.detach() #P to be able to run 100 times
                ...
                #print('from get_all_pts3d, iteration' , i, total_loss)
               
                total_loss.backward(retain_graph=False) #P TRUE could add retain_graph=True to allow backwards multiple times
                pose_optimizer.step()
    ...
    def __call__(self, gt1, gt2, pred1, pred2, trajectory_pred, render_gt=None, **kw):
        name = gt1[0]['label'][0]
        #print('from Eval_NVS: starter __call__ ')

...
class AsymmetricMASt3R(AsymmetricCroCo3DStereo):
    ...
    def forward_coarse_pose(self, view1, view2, enabled=True, dtype=torch.float32): #P bfloat16
        ...
        print('feat1 and feat2', feat1.dtype, feat2.dtype)
    ...
    def forward(self, view1, view2, enabled=True, dtype=torch.float32): #P bfloat16
        if self.wogs:
    ...
    def forward_gs(self, view1, view2, enabled=True, dtype=torch.float32): #P bfloat16
    ...
    def forward_pointmap(self, view1, view2, enabled=True, dtype=torch.float32): #P bfloat16
        # encode the two images --> B,S,D
        batch_size, _, _, _  = view1[0]['img'].shape
        view_num = len(view2)
        ...
        with torch.cuda.amp.autocast(enabled=False, dtype=torch.float32):
            pose_token1_fine = tuple(t.to(torch.float32) for t in pose_token1_fine)
            pose_token2_fine = tuple(t.to(torch.float32) for t in pose_token2_fine)
            self.pose_head_stage2 = self.pose_head_stage2.to(torch.float32)
        ...
        print()
        print(f'wpose is {self.wpose} and the camera embedding is:')
        print(camera_embed)
        print()
